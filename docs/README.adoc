Supplementary Information: Goslin - A Grammar of Succinct Lipid
Nomenclature
============================================================================
...
Figure #fig:goslin-webapp-rest-01[[fig:goslin-webapp-rest-01]]), or ii)
upload a list of lipid names, defined by the user in an interactive form
(see Supplementary
Figure #fig:goslin-webapp-rest-02[[fig:goslin-webapp-rest-02]]). The
...
image:goslin-webapp-form-01[goslin web application submission
form for text files with one lipid name per row.]

image:goslin-webapp-form-02[goslin web application submission
form for user-defined lipid names.]

image:goslin-webapp-form-02a[goslin web application submission
form for user-defined lipid names provides feedback for unknown or
unsupported names and parts thereof.]

image:goslin-webapp-result-03[Parsing results are displayed as
’cards’ for every lipid name. Clicking on a card opens it and shows
details of the according lipid.]

After successful validation, the validated lipids are returned in
overview cards (see Supplementary
Figure #fig:goslin-webapp-rest-03[[fig:goslin-webapp-rest-03]]),
...
Figure #fig:goslin-webapp-rest-04[[fig:goslin-webapp-rest-04]]) .

image:goslin-webapp-result-detail-04[Each result card displays
summary and detail information about a lipid. Depending on the lipid
level, this can include information about each individual fatty acyl.
Cross-links to SwissLipids and LipidMAPS are shown where a normalized
lipid name could be matched unambiguously against the normalized names
of SwissLipids and / or LipidMAPS lipids.]
...
Figure #fig:goslin-webapp-rest-05[[fig:goslin-webapp-rest-05]]). To
illustrate its usage, we will briefly show a small example how a user
can access the rest api with a standard http client.

image:goslin-webapp-rest-05[The goslin web application provides
an interactive documentation for its rest api to simplify programmatic
access.]
...
read Supplementary Section #sec:webServiceUsers[1]. The cppgoslin
...
githubfootnote:[https://github.com/lifs-tools/cppgoslin]. For a detailed
structure of the implementation, read Supplementary
Section #sec:goslinObjectModel[6].
...
developersfootnote:[goslin@lipidomics.at].
...
user, please read Section #sec:webServiceUsers[1]. The pygoslin
implementation has been developed with the following objectives:
...
It is an open-source package under the MIT License available via
githubfootnote:[https://github.com/lifs-tools/pygoslin]. For a detailed
structure of the implementation, read Supplementary
Section #sec:goslinObjectModel[6].
...
Nomenclatures project for the R language
footnote:[https://www.r-project.org/].
...
githubfootnote:[https://github.com/lifs-tools/rgoslin].

Prerequisites
~~~~~~~~~~~~~

This project uses the R programming language. To be able to use it,
please install Rfootnote:[https://cloud.r-project.org/] following the
...
FAQfootnote:[https://cran.r-project.org/web/packages/Rcpp/vignettes/Rcpp-FAQ.pdf],
...
languagefootnote:[https://go.java/].
...
githubfootnote:[https://github.com/lifs-tools/jgoslin].
...
Supplementary Section #sec:goslinObjectModel[6].

[[sec:goslinObjectModel]]
Goslin Object Model
-------------------

image:uml-class-diagram[goslin object model.]

All goslin implementations are implementing the goslin object model as
illustrated in Supplementary
Figure #fig:goslin-object-model[[fig:goslin-object-model]]. The classes
...
see Supplementary Table #tab:lipidlist[[tab:lipidlist]] for details.
...
{empty}[[tab:lipidlist]][tab:lipidlist]
